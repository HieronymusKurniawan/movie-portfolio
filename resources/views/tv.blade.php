...
<body>
    ...
    <div class="w-auto pl-28 pr-10 pt-6 pb-10 grid grid-cols-3 lg:grid-cols-5 gap-5" id="dataWrapper">
        @foreach ($tvShows as $tvShowsItem)
            @php
            ...
            @endphp
            <a href="tv-shows/{{ $tvShowsID }}" class="group">
                <div
                ...
                .done(function(response) {
                    ...
                    if (response.results) {
                        ...
                        response.results.forEach(element => {
                            let original_date = element.first_air_date;
                            let date = new Date(original_date);
                            let tvShowsYear = date.getFullYear();
                            let tvShowsID = element.id;
                            let tvShowsTitle = element.name;
                            let tvShowsImage = `${imageBaseURL}/w500${element.poster_path}`;
                            let tvShowsRating = element.vote_average * 10;

                            htmlData.push(`<a href="tv-shows/${tvShowsID}" class="group">
                <div
                    class="min-w-[232px] min-h-[428px] bg-white drop-shadow-[0_0px_8px_rgba(0,0,0,0.25)] group-hover:drop-shadow-[0_0px_8px_rgba(0,0,0,0.5)] rounded-[32px] p-5 flex flex-col mr-8 duration-100">
                    <div class="overflow-hidden rounded-[32px]">
                        <img src=${tvShowsImage} alt=""
